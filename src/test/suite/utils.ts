...
export async function formatTestFile(filename: string, uri: vscode.Uri): Promise<{ result: string; source: string }> {
...
export function readTestFile(filename: string, uri: vscode.Uri): Promise<string> {
	const extendedUri = uri.with({ path: path.join(uri.fsPath, filename) });
